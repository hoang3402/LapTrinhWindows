...
namespace LTWindows
{
    public partial class Bai4 : Form
    {
        ...
        private void btnGiai_Click(object sender, EventArgs e)
        {
            ...
            {
                var regex = new Regex(@"([+-/*])/");
                var tempA = regex.Match(this.txtbA.Text);
                var tempB = regex.Match(this.txtbB.Text);
                if (tempA.Success || tempB.Success)
                {
                    lblError.Text = "Làm gì có phép -/";
                    return;
                }
